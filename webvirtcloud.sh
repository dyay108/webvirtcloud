...
#/
#/ NON-INTERACTIVE (useful for containers/CI):
#/   Set WVC_NONINTERACTIVE=true to skip prompts and use env defaults:
#/     WVC_FQDN=localhost
#/     WVC_NOVNC_PORT=6080
#/     WVC_NOVNC_PUBLIC_PORT=6080
#/     WVC_NOVNC_HOST=0.0.0.0
#/
#/ CONTAINER BEHAVIOR:
#/   If running inside a container, host-only actions are skipped:
...
in_container=false
if [ -f /.dockerenv ] || grep -qiE '(docker|containerd|kubepods|lxc)' /proc/1/cgroup 2>/dev/null; then
  in_container=true
fi

# Non-interactive defaults via env vars
: "${WVC_NONINTERACTIVE:=false}"
: "${WVC_FQDN:=localhost}"
: "${WVC_NOVNC_PORT:=6080}"
: "${WVC_NOVNC_PUBLIC_PORT:=6080}"
: "${WVC_NOVNC_HOST:=0.0.0.0}"
...
# "clear" is annoying/no-op in CI/containers
if [ "$in_container" != "true" ] && [ -t 1 ]; then
  clear
fi
...
set_firewall () {
  if [ "$in_container" = "true" ]; then
    echo "* Skipping firewall config inside container."
    return 0
  fi
...
set_selinux () {
  if [ "$in_container" = "true" ]; then
    echo "* Skipping SELinux config inside container."
    return 0
  fi
...
set_hosts () {
  if [ "$in_container" = "true" ]; then
    echo "* Skipping /etc/hosts modification inside container."
    return 0
  fi
...
restart_supervisor () {
  if [ "$in_container" = "true" ]; then
    echo "* Skipping systemctl supervisor restart inside container."
    return 0
  fi
...
restart_nginx () {
  if [ "$in_container" = "true" ]; then
    echo "* Skipping systemctl nginx restart inside container."
    return 0
  fi
...
# Collect config (interactive OR env-driven)
fqdn="localhost"

if [ "$WVC_NONINTERACTIVE" = "true" ]; then
  fqdn="$WVC_FQDN"
  novncd_port="$WVC_NOVNC_PORT"
  novncd_public_port="$WVC_NOVNC_PUBLIC_PORT"
  novncd_host="$WVC_NOVNC_HOST"
else
  setupfqdn=default
